 ...
 */
typedef void (*grpc_c_client_callback_t)(grpc_c_context_t *context, void *tag,
                                         int success);
...
struct grpc_c_client_s {
  ...
  grpc_completion_queue
      *queue; /* Completion queue associated with this client */

  grpc_slice host;      /* Hostname of remote providing RPC service */
...
struct grpc_c_context_s {
  ...
  gpr_cv shutdown;
  grpc_c_method_funcs_t
      *method_funcs; /* Pointer to method functions like input/output
                        packer,unpacker, free and method callbacks */
...
typedef void (*grpc_c_memory_free_func_t)(grpc_c_context_t *context,
                                          void *data);

void grpc_c_set_memory_function(grpc_c_memory_alloc_func_t,
                                grpc_c_memory_free_func_t);
 ...
 */
int grpc_c_read(grpc_c_context_t *context, void **content, uint32_t flags,
                long timeout);

int grpc_c_write(grpc_c_context_t *context, void *output, uint32_t flags,
                 long timeout);

int grpc_c_write_done(grpc_c_context_t *context, uint32_t flags, long timeout);

int grpc_c_finish(grpc_c_context_t *context, grpc_c_status_t *status,
                  uint32_t flags);
...
int grpc_c_client_request_sync(grpc_c_client_t *client,
                               ...
                               const char *method_url, void *input,
                               void **output, grpc_c_status_t *status,
                               grpc_c_method_funcs_t *funcs, long timeout);
...
int grpc_c_client_request_async(grpc_c_client_t *client,
                                ...
                                uint32_t flags, const char *method_url,
                                void *input, grpc_c_client_callback_t *cb,
                                void *tag, grpc_c_method_funcs_t *funcs,
                                long timeout);
 ...
 */
grpc_c_server_t *grpc_c_server_create(const char *addr,
 ...
 */
int grpc_c_get_initial_metadata_by_key(grpc_c_context_t *context,
 ...
 */
int grpc_c_get_trailing_metadata_by_key(grpc_c_context_t *context,
                                        const char *key, char *value,
                                        size_t len);
 ...
 */
int grpc_c_add_initial_metadata(grpc_c_context_t *context, const char *key,
                                const char *value);
 ...
 * Returns 0 on success and 1 on failure
 */
int grpc_c_add_trailing_metadata(grpc_c_context_t *context, const char *key,
                                 const char *value);
