 ...
 * data
 */
int grpc_c_stream_read(grpc_call *call, grpc_c_stream_read_t *reader,
                       grpc_byte_buffer **output, uint32_t flags,
                       long timeout) {
  ...
  if (reader->read_wait) {
    ret = gpr_cv_wait(&reader->cv, &reader->lock,
                      grpc_c_deadline_from_timeout(timeout));
    if (ret) {
      gpr_mu_unlock(&reader->lock);
 ...
 */
int grpc_c_stream_write(grpc_call *call, grpc_c_stream_write_t *writer,
                        grpc_byte_buffer *input, uint32_t flags, long timeout) {
  grpc_call_error error;
  grpc_op ops;
  int ret;
  ...
  if (writer->write_wait) {
    ret = gpr_cv_wait(&writer->cv, &writer->lock,
                      grpc_c_deadline_from_timeout(timeout));
    if (ret) {
      gpr_mu_unlock(&writer->lock);
      GRPC_C_ERR("Timeout.");
 ...
 */
int grpc_c_stream_write_done(grpc_call *call, grpc_c_stream_write_t *writer,
...
grpc_c_stream_status_t *grpc_c_status_init(int is_client) {
  ...
  status =
      (grpc_c_stream_status_t *)grpc_malloc(sizeof(grpc_c_stream_status_t));
  if (NULL == status) {
    GRPC_C_ERR("Nomemory.");
    return NULL;
  }
  ...
  status->result = GRPC_C_ERR_NORECV;
...
void grpc_c_status_destory(grpc_c_stream_status_t *status) {
  ...
  grpc_slice_unref(status->status_details);
...
int grpc_c_status_trailing_metadata_set(grpc_c_stream_status_t *status,
                                        const char *key, const char *value) {

  int ret;
  gpr_mu_lock(&status->lock);

  if (status->result != GRPC_C_ERR_NORECV) {
    gpr_mu_unlock(&status->lock);
    GRPC_C_ERR("trailing metadata has been sent.");
    return GRPC_C_ERR_FAIL;
  }

  ret = grpc_c_metadata_array_add(&status->trailing_metadata, key, value);
  gpr_mu_unlock(&status->lock);

  return GRPC_C_OK;
}

int grpc_c_status_trailing_metadata_get(grpc_c_stream_status_t *status,
  ...
  gpr_mu_lock(&status->lock);
  if (status->result != GRPC_C_OK) {
    gpr_mu_unlock(&status->lock);
    GRPC_C_ERR("trailing metadata has not accepted.");
    return GRPC_C_ERR_FAIL;
  }

  ret = grpc_c_metadata_array_get(&status->trailing_metadata, key, value, len);
  gpr_mu_unlock(&status->lock);
 ...
 */
int grpc_c_status_send(grpc_call *call, grpc_c_stream_status_t *status,
                       grpc_c_status_t *status_input, uint32_t flags) {
  grpc_call_error error;
  grpc_op ops;
  int ret;
  ...
  if (status_input->message[0] == '\0') {
    ...
    status->status_details =
        grpc_slice_from_static_string(status_input->message);
  }
  ...
  ops.data.send_status_from_server.trailing_metadata_count =
      status->trailing_metadata.count;
  ops.data.send_status_from_server.trailing_metadata =
  ...
  (void)gpr_cv_wait(&status->cv, &status->lock,
                    grpc_c_deadline_from_timeout(-1));
 ...
 */
int grpc_c_status_recv(grpc_call *call, grpc_c_stream_status_t *status,
                       grpc_c_status_t *status_output, uint32_t flags) {
  grpc_call_error error;
  grpc_op ops;
  int ret;
...
int grpc_c_initial_metadata_set(grpc_c_initial_metadata_t *init_metadata,
                                const char *key, const char *value) {
  ...
  if (init_metadata->done_once) {
    ...
    GRPC_C_ERR("initial metadata has been sent.");
    return GRPC_C_ERR_FAIL;
  }

  ret = grpc_c_metadata_array_add(&init_metadata->metadata, key, value);
  gpr_mu_unlock(&init_metadata->lock);

  return ret;
}

int grpc_c_initial_metadata_get(grpc_c_initial_metadata_t *init_metadata,
  ...
  if (!init_metadata->done_once) {
    gpr_mu_unlock(&init_metadata->lock);
    GRPC_C_ERR("initial metadata has not accepted.");
    return GRPC_C_ERR_FAIL;
  }

  ret = grpc_c_metadata_array_get(&init_metadata->metadata, key, value, len);
  gpr_mu_unlock(&init_metadata->lock);
...
grpc_c_initial_metadata_t *grpc_c_initial_metadata_init(int is_send) {
  ...
  init_metadata = (grpc_c_initial_metadata_t *)grpc_malloc(
      sizeof(grpc_c_initial_metadata_t));
...
void grpc_c_send_initial_metadata_event_cb(grpc_c_event_t *event, int success) {
  grpc_c_initial_metadata_t *init_metadata =
      (grpc_c_initial_metadata_t *)event->data;
...
int grpc_c_send_initial_metadata(grpc_call *call,
                                 grpc_c_initial_metadata_t *init_metadata,
                                 long timeout) {
  grpc_call_error error;
  grpc_op ops;
  ...
  error =
      grpc_call_start_batch(call, &ops, 1, (void *)&init_metadata->event, NULL);
  if (error != GRPC_CALL_OK) {
    gpr_mu_unlock(&init_metadata->lock);
    GRPC_C_ERR("Failed to finish send status ops batch");
    return GRPC_C_ERR_FAIL;
...
int grpc_c_recv_initial_metadata(grpc_call *call,
                                 grpc_c_initial_metadata_t *init_metadata,
                                 long timeout) {
  grpc_call_error error;
  grpc_op ops;
  ...
  ops.data.recv_initial_metadata.recv_initial_metadata =
...
int grpc_c_server_recv_close(grpc_call *call, grpc_c_recv_close_t *recv_close) {
  ...
  error =
      grpc_call_start_batch(call, &ops, 1, (void *)&recv_close->event, NULL);
  if (error != GRPC_CALL_OK) {
...
int grpc_c_server_recv_close_wait(grpc_c_recv_close_t *recv_close) {
  ...
  if (recv_close->result == GRPC_C_ERR_NORECV) {
    (void)gpr_cv_wait(&recv_close->cv, &recv_close->lock,
                      grpc_c_deadline_from_timeout(-1));
  }
