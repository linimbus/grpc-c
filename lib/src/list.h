...
#define GRPC_LIST_EMPTY(head) ((head)->next == (head))

#define GRPC_LIST_NEXT(item) ((item)->next)

#define GRPC_LIST_BEFORE(item) ((item)->prev)
...
#define GRPC_LIST_TRAVERSAL(pstItem, pstHead)                                  \
  for ((pstItem) = (pstHead)->next; (pstItem) != (pstHead);                    \
