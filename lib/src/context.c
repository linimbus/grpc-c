...
int grpc_c_read(grpc_c_context_t *context, void **content, uint32_t flags,
  ...
  ret = grpc_c_stream_read(context->call, context->reader, &output, flags,
                           timeout);
  gpr_mu_unlock(&context->lock);
...
int grpc_c_write(grpc_c_context_t *context, void *output, uint32_t flags,
  ...
  grpc_c_send_initial_metadata(context->call, context->send_init_metadata,
                               timeout);
  ...
  ret = grpc_c_stream_write(context->call, context->writer, input, flags,
                            timeout);
...
int grpc_c_write_done(grpc_c_context_t *context, uint32_t flags, long timeout) {
  ...
  ret =
      grpc_c_stream_write_done(context->call, context->writer, flags, timeout);
  gpr_mu_unlock(&context->lock);
...
int grpc_c_finish(grpc_c_context_t *context, grpc_c_status_t *status,
  ...
  if (context->is_client) {
    ...
    ret = grpc_c_send_initial_metadata(context->call,
                                       context->send_init_metadata, -1);
    if (ret != GRPC_C_OK) {
      goto failed;
    }
...
int grpc_c_get_initial_metadata_by_key(grpc_c_context_t *context,
  ...
  ret =
      grpc_c_initial_metadata_get(context->recv_init_metadata, key, value, len);
  gpr_mu_unlock(&context->lock);
