...
extern int grpc_c_register_method_ready(grpc_c_server_t *server,
                                        grpc_c_method_t *np);
...
void grpc_c_server_register_event_cb(grpc_c_event_t *event, int success) {
  ...
  grpc_c_register_method_ready(context->type.server.server_t,
                               context->type.server.method);

  if (!success) {
    grpc_c_context_free(context);
  } else {
    grpc_c_thread_pool_add(server->thread_pool, grpc_c_server_call_start,
                           (void *)context);
  }
}
...
int grpc_c_register_method_ready(grpc_c_server_t *server, grpc_c_method_t *np) {
  ...
  error = grpc_server_request_registered_call(
      server->server, np->method_tag, &context->call, &context->deadline,
      &context->recv_init_metadata->metadata, NULL, server->queue,
      server->queue, &context->type.server.event);
...
int grpc_c_register_method(grpc_c_server_t *server, const char *method_url,
  ...
  method_tag = grpc_server_register_method(
      server->server, method_url, server->hostname, GRPC_SRM_PAYLOAD_NONE, 0);
  if (method_tag == NULL) {
 ...
 */
int grpc_c_server_add_http2_port(grpc_c_server_t *server, const char *addr,
                                 grpc_server_credentials *creds) {
  if (server == NULL)
    return 0;

  return grpc_server_add_http2_port(server->server, addr, creds);
}
...
grpc_c_server_t *grpc_c_server_create(const char *addr,
   ...
   */
  ret = grpc_server_add_http2_port(server->server, addr, creds);
  if (ret == 0) {
...
void grpc_c_server_master_start(void *arg) {
  ...
  for (;;) {
    event = grpc_completion_queue_next(
        server->queue, gpr_inf_future(GPR_CLOCK_MONOTONIC), NULL);
...
int grpc_c_server_start(grpc_c_server_t *server) {
   ...
   */
  grpc_c_thread_pool_add(server->thread_pool, grpc_c_server_master_start,
                         server);
...
void grpc_c_server_wait(grpc_c_server_t *server) {
  gpr_mu_lock(&server->lock);
  gpr_cv_wait(&server->shutdown_cv, &server->lock,
...
int grpc_c_server_stop(grpc_c_server_t *server) {
  ...
  grpc_server_shutdown_and_notify(server->server, server->queue,
