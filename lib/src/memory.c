...
void *grpc_malloc(size_t size) { return malloc(size); }

void *grpc_realloc(void *ptr, size_t size) { return realloc(ptr, size); }

void grpc_free(void *data) { free(data); }

void *grpc_c_malloc(grpc_c_context_t *context, size_t size) {
  return grpc_malloc(size);
}

void grpc_c_free(grpc_c_context_t *context, void *data) { grpc_free(data); }
 ...
 */
void grpc_c_set_memory_function(grpc_c_memory_alloc_func_t allocfunc,
                                grpc_c_memory_free_func_t freefunc) {
