...
int grpc_c_client_request_sync(grpc_c_client_t *client,
  ...
  context->call = grpc_channel_create_call(
      client->channel, NULL, 0, client->queue,
      grpc_slice_from_static_string(method_url), &client->host,
  ...
  ret = grpc_c_send_initial_metadata(context->call, context->send_init_metadata,
                                     timeout);
  if (ret != GRPC_C_OK) {
    goto failed;
  }
  ...
  ret = grpc_c_recv_initial_metadata(context->call, context->recv_init_metadata,
                                     timeout);
  if (ret != GRPC_C_OK) {
    goto failed;
  }
...
int grpc_c_client_connect_status_sub(grpc_c_client_t *client) {
  ...
  if (client->channel) {
    ...
    client->connect_status =
        grpc_channel_check_connectivity_state(client->channel, 0);
    ...
    grpc_channel_watch_connectivity_state(
        client->channel, client->connect_status,
        gpr_inf_future(GPR_CLOCK_REALTIME), client->queue,
...
void grpc_c_client_master_task(void *arg) {
  ...
  for (;;) {
    event = grpc_completion_queue_next(
        client->queue, gpr_inf_future(GPR_CLOCK_MONOTONIC), NULL);
...
grpc_c_client_t *grpc_c_client_init(const char *server_name,
  ...
  client->channel =
      grpc_channel_create(server_name, channel_creds, channel_args);
  ...
  if (client->queue == NULL) {
    GRPC_C_ERR("Failed to create completion queue for server "
  ...
  grpc_c_thread_pool_add(client->thread_pool, grpc_c_client_master_task,
                         client);
...
int grpc_c_client_stop(grpc_c_client_t *client) {
  ...
  do {
    event = grpc_completion_queue_next(
        client->queue, gpr_inf_future(GPR_CLOCK_REALTIME), NULL);
...
void grpc_c_client_wait(grpc_c_client_t *client) {
  gpr_mu_lock(&client->lock);
  if (!client->shutdown) {
    (void)gpr_cv_wait(&client->shutdown_cv, &client->lock,
